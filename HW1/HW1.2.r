...
p7 = ggplot(data = Air_delay_arrival)+
  geom_bar(aes(x = reorder(airport, Delay), y = Delay),stat='identity',position='dodge', fill = "pink")+
  coord_flip()
p7
Air_delay_departure = subset(Air_summ2, (Air_summ2$INorOUT == "Departure"))
p8 = ggplot(data = Air_delay_departure)+
  geom_bar(aes(x = reorder(airport, Delay), y = Delay),stat='identity',position='dodge', fill = "light blue")+
  coord_flip()
p8
