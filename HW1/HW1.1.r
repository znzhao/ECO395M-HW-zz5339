...
p1 = ggplot(data = greenbuildings) + 
  geom_point(mapping = aes(x = leasing_rate, y = Rent),alpha = 0.3, col = brewer.pal(6, "Greens")[5])+
...
p6 = ggplot(GB_summ, aes(x=size_category, y=Rent_mean)) + 
  ...
  scale_fill_brewer(palette= "Greens",direction = -1)+
  facet_wrap(~ stories_category)+
  labs(title = "Mean of Different Size Groups")+
  theme(plot.title = element_text(hjust = 0.5))
p6
