...
p6 = ggplot(GB_summ_a, aes(x=amenities, y=Rent_mean,fill=Green)) + 
  ...
  scale_fill_discrete(name = "Green Buildings or Not")+
  scale_x_continuous(breaks = c(0,1),labels = c("0" = "No", "1" = "Yes"))+
  labs(title = "Rent for Different Stories and Access of Amenities",
       x = "Are Amenities Available?", y = "Rent ($/sqft)")+
  theme_bw()+
...
p8 = ggplot(GB_summ2,aes(x = stories_category, y = leasing_rate, fill = Green))+
  ...
  labs(title = "Stories vs Occupancy Rate",
       y = "Occupancy Rate (%)",
       x = "Stories")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), legend.position="bottom")
