...
```{r model2.2.2, echo = FALSE, warning=FALSE}
...
accrate = do(Ntimes)*{
  ...
  # lm test
  lm_result = lm(shares ~ n_tokens_title + n_tokens_content + num_hrefs + 
  ...
  sharehat_test = predict(lm_result, DF_test)
  yhat_test1 = ifelse((sharehat_test>1400), yes = 1 ,no = 0)
  ar1 = sum(yhat_test1 == y_test)/n_test
  ...
  yhat_test2 = ifelse((phat_test>1400), yes = 1 ,no = 0)
  ...
  c(ar1, ar2, ar3, tpr3, fpr3)
  }
colnames(accrate) <- c("share-LM", "viral-LM", "GLM", "tpr_GLM", "fpr_GLM")
kable(colMeans(accrate)[1:3], col.names = "Accurate Rate")
```
...
kable(t, caption = "Table 2.1.3: Confusion Matrix of the Baseline Model",padding = 2, align = "c", col.names = c("prediction = 0", "prediction = 1"))
```
...
for(i in c(1:run_time)){
  ...
  for(j in c(1:K)){
    ...
    # KNN - share
    knn_share = FNN::knn.reg(train=Xtilde_train, test= Xtilde_test, y = share_train, k=m)
    ypred_knn = knn_share$pred
    KNN_train_test2 = ifelse(ypred_knn > 1400, 1, 0)  
    confusion_in_KNN2 = table(y = OnlineNews_test$viral, yhat = KNN_train_test2)
    accuracy_rate2 = sum(diag(confusion_in_KNN2))/sum(confusion_in_KNN2)
    myList_share[j]= myList_share[j] + accuracy_rate2
  }
  # fit the model
...
```{r share2.3.3, echo = FALSE, warning=FALSE}
myList_share = myList_share/run_time
accrate2 <- data.frame(myList_share, myL)
ggplot(data = accrate2, aes(x = myL, y = myList_share))+
  geom_point(shape = "O")+
  geom_line(col = "red")+
  geom_vline(xintercept = myL[which.max(myList_share)])+
  labs(title = "AVG Accuracy Rate vs K (5<K<100) Model: KNN regression",
       x = "K",
       y = "AVG Accuracy Rate")+
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r table2.3.4, echo = FALSE, warning=FALSE}
knn_model = knn.reg(Xtilde_train, Xtilde_test, ytrain, k=myL[which.max(myList)])
KNN_train_test1 = ifelse(knn_model$pred > 0.5, 1, 0)
    
confusion_in_KNN = table(y = OnlineNews_test$viral, yhat = KNN_train_test1)
df = as.data.frame(confusion_in_KNN)
t = data.frame(yhat0 = c(df$Freq[1],df$Freq[2]),yhat1 = c(df$Freq[3],df$Freq[4]))
row.names(t) <- c("viral = 0", "viral = 1")
kable(t, caption = "Table 2.1.3: Confusion Matrix of the Baseline Model",padding = 2, align = "c", col.names = c("prediction = 0", "prediction = 1"))
```
